# Stretch - ground the brief in your real Azure footprint (~5 min)

**Only do this if your core brief and the GitHub-issue stretch are both done.**
...
> **Prereqs.** An Azure subscription you can `az login` to and write-equivalent RBAC for reads. If `az login` is locked down on your laptop or you cannot install the Azure CLI, skip this tier. Some enterprises also require tenant admin consent for certain MCP servers - if you hit that block, do not try to work around it during the workshop.
